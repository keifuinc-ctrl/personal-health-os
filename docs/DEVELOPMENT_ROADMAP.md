...
- [ ] **監査ログ機能の拡張**
  - [ ] `lib/services/audit-log.ts` の拡張（`logAction`関数追加）
  - [ ] すべてのデータ操作を不変ログに記録
...
- [ ] ナビゲーション/フッター/レスポンシブ
- [ ] ローディング/エラー表示/アクセシビリティ
- [ ] Beneficiary Platform 専用UI（`/beneficiary/*`ページ群）
- [ ] アニメーション/トランジション効果
- [ ] 3D要素（必要に応じて）
- [ ] ユーザビリティテスト
...
- [ ] 基本ログ
- [ ] エラー対応フロー（手順書）（[ERROR_HANDLING.md](./ERROR_HANDLING.md)）
...
- 自分カルテ: 0%（基本CRUD/検索/インポート/エクスポート、外部カルテ連携が未実装）
- 健康計画: 0%（集計/表示/履歴、リスク予測、予防計画、ビッグデータ分析が未実装）
- チーム&地域包括ケア: 0%（招待制/検索/投稿、支援事業所検索機能が未実装）
- マイページ: 0%（通知/設定/プラン管理、外部カルテ連携設定が未実装）

#### 共通基盤機能
- 電子保存の三原則: 0%（デジタル署名、改ざん検知、長期保存形式、バックアップ、データ移行が未実装）
...
- [ ] **コア機能の完成**
  - [ ] 統合カルテ機能（薬、病院、疾患、検査データの記録・管理）
  - [ ] AI生活計画機能（健康データ記録、リスク予測、予防計画、運動計画）
